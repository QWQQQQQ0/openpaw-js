...
usage: |
  ...
  **Visual fallback**: If uia_get_interactive returns empty (custom-drawn UIs like QQ音乐), use desktop_screenshot → ocr_recognize → desktop_click with OCR coordinates.
  ...
  | Priority | Tools | When |
  |----------|-------|------|
  | 1. Visual | desktop_screenshot, desktop_screenshot_window, desktop_screenshot_region, desktop_ocr | See what's on screen |
  | 2. Input | desktop_click, desktop_double_click, desktop_right_click, desktop_type, desktop_press_key, desktop_drag, desktop_scroll, desktop_move_mouse | Mouse & keyboard |
...
usage_cn: |
  ...
  **视觉兜底**：当 uia_get_interactive 返回空（自定义绘制 UI，如 QQ 音乐）时，使用 desktop_screenshot → ocr_recognize → desktop_click 配合 OCR 坐标点击。
  ...
  | 2. 输入 | desktop_click, desktop_double_click, desktop_right_click, desktop_type, desktop_press_key, desktop_drag, desktop_scroll, desktop_move_mouse | 鼠标键盘操作 |
...
[
  {
    ...
    "description": "Take a screenshot. If window_hwnd is provided, captures only that window (recommended when target window is known — smaller image, less noise). Without window_hwnd, captures the full desktop.",
    "name_cn": "截图",
    "description_cn": "截取屏幕截图。提供 window_hwnd 时仅截取目标窗口（推荐——图片更小、噪音更少）。不提供时截取全屏。",
    "parameters": {
      "type": "object",
      "properties": {
        "window_hwnd": { "type": "integer", "description": "Window handle to capture specific window (optional). If omitted, captures full desktop." }
      }
    }
  },
  {
  ...
  },
  {
    ...
    "description": "Capture a screenshot of a specific window by its handle. Returns base64 image data.",
    "name_cn": "窗口截图",
    "description_cn": "截取指定窗口的截图，返回 base64 图片数据。",
    "parameters": {
      "type": "object",
      "properties": {
        "hwnd": { "type": "integer", "description": "Window handle" }
  ...
  {
    "name": "desktop_click",
    "description": "Click at absolute screen coordinates (x, y). Use uia_click instead when possible.",
    "name_cn": "桌面点击坐标",
    "description_cn": "在屏幕绝对坐标 (x, y) 处点击。优先使用 uia_click。",
    "parameters": {
      "type": "object",
      "properties": {
  ...
  },
  {
    ...
    "name_cn": "双击坐标",
    "description_cn": "在屏幕绝对坐标 (x, y) 处双击。",
    "parameters": {
      "type": "object",
      "properties": {
        "x": { "type": "integer" },
  ...
  },
  {
    ...
    "name_cn": "右键点击",
    "description_cn": "在屏幕绝对坐标 (x, y) 处右键点击。",
    "parameters": {
      "type": "object",
      "properties": {
        "x": { "type": "integer" },
  ...
  },
  {
    ...
    "description": "Middle-click at absolute screen coordinates (x, y). Commonly used to open links in new tabs or close browser tabs.",
    "name_cn": "中键点击",
    "description_cn": "在屏幕绝对坐标 (x, y) 处中键点击。常用于在新标签页打开链接或关闭浏览器标签。",
    "parameters": {
      "type": "object",
      "properties": {
        "x": { "type": "integer" },
